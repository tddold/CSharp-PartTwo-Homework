    ...
    public class Calculator
    {
        ...
        public static void StartCalculating()
        {
            ...
            while (input != "end")
            {
                try
                {
                    ...
                    var seperatedTokens = SeparateTokens(trimmedInput);

                    var reversePolishNotation = ConversToReversePolishNotation(seperatedTokens);
        ...
        public static double GetResultFromRPN(Queue<string> queue)
        {
            ...
            while (queue.Count != 0)
            {
                ...
                else if(arithmethicOperations.Contains(currentToken[0]) || functions.Contains(currentToken))
                {
                    if (currentToken == "+")
                    {
                        if (stack.Count < 2)
        ...
        public static Queue<string> ConversToReversePolishNotation(List<string> tokens)
        {
            ...
            for (int i = 0; i < tokens.Count; i++)
            {
                ...
                else if (arithmethicOperations.Contains(currentToken[0])) // if not working => refactor logic for current logic
                {
                    while (stack.Count != 0 && arithmethicOperations.Contains(stack.Peek()[0]) && Precedence(currentToken) <= Precedence(stack.Peek()))
                    {
                        queue.Enqueue(stack.Pop());
                    }
